...
/*	인터페이스 특징 :
	- 인터페이스의 모든 메서드는 추상메소드(public abstract)로 선언해야됨. 멤버 변수는 상수(public static final) 정의 가능
  	  (생략해도 컴파일 과정에서 자동 생성)
	- interface는 다중 상속 가능
	- JDK 1.8부터 default methods 사용 가능 -> 인터페이스에 메소드 추가할 시 모든 구현체에 코드 작성 과정을 줄이기 위해 추가.
